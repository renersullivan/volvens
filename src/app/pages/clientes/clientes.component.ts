...
export class ClientesComponent implements OnInit {
  ...
  public requestPage(page?: number, itemPerPage?: string | null) {
    ...
    if(this.city){
      this.http.get<Clientes>(`http://api/clientesByCity?page=${this.page}&limit=${this.itemPerPage}&orderBy=cidade&filter=${this.city}`).subscribe(
        result => {
          console.log(result);
    ...
    this.http.get<Clientes>(`http://api:3333/clientes?page=${this.page}&limit=${this.itemPerPage}&orderBy=cidade`).subscribe(
      result => {
        console.log(result);
        this.results = result.data;
        this.metaData = result.meta
